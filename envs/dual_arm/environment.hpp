...
public:
    struct Parameters {
        ...
        Eigen::Vector2f target_pos_{0.0, 0.4} ;
    ...
    int64_t stateSpaceDim() const {
        return box_names_.size() * 4 ;
    }

    int64_t goalSpaceDim() const {
        return 2 ;
    }
    ...
    Eigen::VectorXf desiredGoal() const ;

    Eigen::VectorXf stateToGoal(const State &state) const ;

    float computeRewardForGoal(const State &state, const Eigen::VectorXf &goal) ;
